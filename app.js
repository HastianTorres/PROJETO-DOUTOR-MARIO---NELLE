...
let medicines = [];
let log = [];
let currentDayOnScreen = todayKey();
...
function checkDayChange() {
  ...
  if (realToday !== currentDayOnScreen) {
    currentDayOnScreen = realToday;

    // Limpa apenas os avisos já disparados na sessão.
    // O histórico continua salvo no Firebase.
    sessionStorage.clear();

    // Recarrega a tela para mostrar os remédios do novo dia como pendentes.
    render();
  }
}
...
$("exportCsv").addEventListener("click", () => {
  ...
  if (rows.length === 1) {
    rows.push(["", "", "Nenhuma dose foi marcada como Tomei ainda", "", "", "", "", ""]);
  }

  downloadCsv(`historico-doses-${todayKey()}.csv`, rows);
});

$("exportMedicinesCsv").addEventListener("click", () => {
  const rows = [["remedio", "instrucao", "horarios", "frequencia", "data_inicial", "observacao"]];
  ...
  medicines.forEach(med => {
    rows.push([
      ...
      med.times.join(" | "),
      frequencyLabel(med),
      med.startDate || todayKey(),
      med.obs || ""
    ]);
  });

  if (rows.length === 1) {
    rows.push(["Nenhum remédio cadastrado", "", "", "", "", ""]);
  }

  downloadCsv(`remedios-cadastrados-${todayKey()}.csv`, rows);
});

$("exportScheduleCsv").addEventListener("click", () => {
  const rows = [["data", "horario", "remedio", "instrucao", "frequencia", "status", "marcado_as", "observacao"]];
  const today = new Date();
  ...
  for (let i = 0; i < 30; i++) {
    ...
    medicines
      .filter(med => isMedicineDueOn(med, key))
      .forEach(med => {
        med.times.forEach(time => {
          const record = getLog(key, med.id, time);
          rows.push([
            key,
            time,
            med.name,
  ...
  if (rows.length === 1) {
    rows.push(["", "", "Nenhum remédio programado para os próximos 30 dias", "", "", "", "", ""]);
  }

  downloadCsv(`agenda-30-dias-${todayKey()}.csv`, rows);
});
